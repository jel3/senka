...
/// Read and parse `env/{name}.yml`.
pub fn load_env(root: &Path, name: &str) -> Result<Environment, LoadError> {
    let path = root.join("env").join(format!("{name}.yml"));
    let contents = std::fs::read_to_string(&path).map_err(|e| LoadError::ReadFile {
        path: path.clone(),
        source: e,
    })?;
...
/// Read and parse `requests/{name}.yml`.
pub fn load_request(root: &Path, name: &str) -> Result<RequestDef, LoadError> {
    let path = root.join("requests").join(format!("{name}.yml"));
    let contents = std::fs::read_to_string(&path).map_err(|e| LoadError::ReadFile {
        path: path.clone(),
        source: e,
    })?;
...
/// List environment names (filenames in `env/` without `.yml` extension).
pub fn list_envs(root: &Path) -> Result<Vec<String>, LoadError> {
    list_yml_stems(&root.join("env"))
}

/// List request names (filenames in `requests/` without `.yml` extension).
pub fn list_requests(root: &Path) -> Result<Vec<String>, LoadError> {
    list_yml_stems(&root.join("requests"))
}
